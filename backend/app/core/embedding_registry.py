...
class EmbeddingModelPreset:
    ...
    dim: int                     # 向量维度
    max_tokens: int = 512        # 模型最大 token 数
    chunk_size: int = 500        # 分块字符数（起点，动态调整会自动优化）
...
EMBEDDING_MODEL_PRESETS: List[EmbeddingModelPreset] = [
    EmbeddingModelPreset(
        id="yuan2-hf",
        display_name="Yuan-embedding-2.0-zh (HF)",
        backend_type="huggingface",
        model_name_or_path="IEITYuan/Yuan-embedding-2.0-zh",
        persist_subdir="yuan2",
        dim=1792,
        max_tokens=512,
        is_default=True,
        description="榜单前排，300M小参数，高维向量检索",
    ),
    EmbeddingModelPreset(
        id="qwen3-hf",
    ...
    EmbeddingModelPreset(
        id="jasper-hf",
        display_name="Jasper-Token-Compression-600M (HF)",
        backend_type="huggingface",
        model_name_or_path="infgrad/Jasper-Token-Compression-600M",
        persist_subdir="jasper",
        dim=2048,
        max_tokens=32768,
        description="榜单前排 600M 模型，性能优于 Qwen3，支持超长文本",
    ),
    EmbeddingModelPreset(
        id="qwen3-gguf",
